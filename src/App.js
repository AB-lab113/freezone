...
createAppKit({
  ...
  metadata: { name: "FreeZone", description: "Forum décentralisé, libre et privé", url: "https://freezone-kappa.vercel.app", icons: ["https://freezone-kappa.vercel.app/favicon.ico"] },
  themeMode: "dark",
  themeVariables: { "--w3m-accent": "#6366f1", "--w3m-border-radius-master": "8px", "--w3m-font-family": "Inter, sans-serif" }
});
    ...
    const init = async () => {
      try {
        ...
        if (existing) {
          ...
          setMembres(updated); localStorage.setItem("freezone_membres", JSON.stringify(updated));
        } else {
    ...
    <div>
      ...
      <header className="header">
        ...
        <div className="header-actions">
          ...
          {account && <button className="btn btn-ghost" onClick={() => setPage("profil")} style={{ fontSize: 16 }}>⚙️</button>}
          {account ? (
        ...
        <div className="forum-page">
          ...
          <div style={{ borderRadius: 16, padding: 28, marginBottom: 24, background: dark ? "#161b22" : "#ffffff", border: `1.5px solid ${dark ? "#30363d" : "#e2e8f0"}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[
                { label: "STATUT", value: estAbonne ? "Actif ✅" : "Inactif ⚠️", color: estAbonne ? "#22c55e" : "#f59e0b" },
      ...
      {showNewSalon && (
        <div style={{ position: "fixed", inset: 0, background: "#0008", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: dark ? "#161b22" : "white", borderRadius: 16, padding: 32, width: 420, border: "1.5px solid #6366f1" }}>
            <h2 style={{ marginBottom: 24, color: "#6366f1" }}>🏛️ Nouveau salon</h2>
